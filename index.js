...
const cd = 4.32e+7
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./data/functions')
const { fetchJson } = require('./lib/fetcher')
const { ryn } = require('./anjay')
...
limitawal = 30
memberlimit = 5
cr = "Bot sudah terverifikasi whatsapp"
...
console.log(color('[','white'),color('!','red'),color(']','white'),color('qr already scan.subscribe','green'))
})
...
teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* /n*INTRO DULU KAK*/n*Nama:*/n*Umur:*/n*Kota:*/nDan salken ya kak semoga betah di sini`
...
teks = `*Selamat tinggal kak*: @${num.split('@')[0]} /n*Hati Hati di jalan nanti diculik om om*`
let buff = await getBuffer(ppimg)
client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
}
} catch (e) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            try {
		        ...
		        if (isGroupAdmins) return reply('Admin bebas kalo mau send link')
		        client.updatePresence(from, Presence.composing)
		        if (mesejAnti.includes("#izinbos")) return reply("Iya kak jangan spam ya")
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		        reply(`Kak ${sender.split("@")[0]} Grup ini ada antilink nya siap siap ku kick`)
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*JADIKAN BOT SEBAGAI ADMIN!*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Piww... Yey headsod!!")
		        }, 2000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Bismillahirrahmanirrahim")
		        }, 1000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Siap??")
...
switch(command) {
                case 'help':
				case 'menu':
				if (isBanned) return reply(ryn.baned())
				if (!isRegistered) return reply(ryn.noregis())
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
				...
				//		reply(ryn.cmdnf(prefix, command))
					} else {
